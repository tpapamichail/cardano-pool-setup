...
Options:
  -f, --fast          Skip time-consuming checks (external port test)
      --json          Output results as JSON
      --no-telegram   Do not send Telegram alert
  -h, --help          Show help

Environment:
  RELAY_CONTAINER         Container name (default: relay)
  BP_EXPECTED_IP          BP IP for connectivity check (optional)
  PEER_RELAY_IPS          Comma-separated other relays expected to connect
...
TIP_LAG_OK=60
TIP_LAG_WARN=180
MIN_INBOUND_PEERS=2     # at least BP + 1 other
MIN_OUTBOUND_PEERS=5    # at least 5 ledger peers
DB_GROWTH_MIN_KB=10     # if it doesn't grow >10KB in 30s → stuck

# Optional: peer IPs we expect
...
if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER}$"; then
  err "Relay container '${CONTAINER}' is not running"
  if [[ "$OUTPUT_FORMAT" == "text" ]]; then
    echo
    echo -e "${R}${B} ✗ CRITICAL${N} — relay is down\n"
  fi
  exit 1
fi
...
if [[ -n "$started" ]]; then
  ...
  if [[ $uptime_sec -lt 300 ]]; then
    warn "Relay just started (uptime: ${uptime_sec}s) — possible recent restart"
  ...
  warn "Could not read uptime"
fi

# Restart count — if it restarts often, there is a problem
restarts=$(docker inspect "$CONTAINER" --format '{{.RestartCount}}' 2>/dev/null)
if [[ -n "$restarts" ]]; then
  if [[ ${restarts:-0} -gt 5 ]]; then
    warn "Restart count: ${restarts} — check docker logs for crashes"
  else
...
if [[ -n "$validation" ]]; then
  ...
  # Check if it is recent in time
  last_line_time=$(echo "$logs_recent" | grep "Validated chunk" | tail -1 | awk '{print $1, $2}')
  warn "Chain validation in progress: chunk ${chunk}/${total} (${progress}%)"
  hint "Last validation log: ${last_line_time}"
  hint "Node is not ready yet — waiting for validation"
else
  ok "Chain validation complete (or already ran)"
fi

# Also check for db corruption signals
corruption=$(echo "$logs_24h" | grep -ciE "corrupted|inconsistent|database error|chainDB.*Error" || true)
if [[ ${corruption:-0} -gt 0 ]]; then
  err "${corruption} DB corruption signals in 24h — check logs"
fi
...
tip=$(QUERY_TIP)
if [[ -z "$tip" ]]; then
  warn "Cannot query tip (possibly still in validation)"
  tip='{}'
fi
...
# Tip lag (same calculation as BP)
wall_now=$(date +%s)
expected_slot=$((wall_now - SHELLEY_START_UNIX + BYRON_END_SLOT))
tip_lag=$((expected_slot - slot))

if [[ $slot -eq 0 ]]; then
  hint "Tip lag not calculated (no slot yet)"
elif [[ $tip_lag -lt $TIP_LAG_OK ]]; then
  ok "Chain tip lag: ${tip_lag}s"
elif [[ $tip_lag -lt $TIP_LAG_WARN ]]; then
  warn "Chain tip lag: ${tip_lag}s — borderline"
else
  err "Chain tip lag: ${tip_lag}s — relay is losing tip, BP will not receive blocks in time"
fi

# Density — if significantly different from normal 0.05 = problem
density=$(EXEC_RELAY curl -s "${CONTAINER_METRICS_URL}" 2>/dev/null | \
  awk '/^cardano_node_metrics_density_real/ {print $2}')
if [[ -n "$density" ]]; then
  # Compare with 0.05 (average) - acceptable 0.03-0.07
  density_int=$(awk -v d="$density" 'BEGIN {printf "%d", d*1000}')
  if [[ $density_int -ge 30 ]] && [[ $density_int -le 70 ]]; then
    ok "Chain density: ${density} (normal)"
  else
    warn "Chain density: ${density} — outside normal range 0.030-0.070"
  fi
fi
...
if command -v chronyc >/dev/null 2>&1 && systemctl is-active --quiet chronyd 2>/dev/null; then
  ...
  if [[ ${abs_ms:-9999} -lt 100 ]]; then
    ...
    warn "NTP offset ${offset_sec}s — borderline for relay"
  else
    err "NTP offset ${offset_sec}s — block propagation will drift"
  fi
  ...
  warn "No NTP service active — apt install chrony"
fi
...
if [[ $disk_pct -lt 80 ]]; then
  ...
  warn "Disk free: ${disk_avail} (${disk_pct}% used) — clean up soon"
else
  err "Disk ${disk_pct}% used — CRITICAL"
fi
...
if [[ ${mem_int:-0} -lt 75 ]]; then
  ...
  warn "Memory: ${mem_pct}% — close to limit"
else
...
if [[ -z "$metrics" ]]; then
  ...
  # ── Connection counts ──
  # P2P-first (newer versions), legacy fallback
  ...
  # Strict awk match with trailing space to avoid matching HotBigLedgerPeers etc.
  ...
  # ── Inbound governor breakdown (new in P2P) ──
  ...
  if [[ $inb -ge $MIN_INBOUND_PEERS ]]; then
    ok "Inbound: ${inb} connections (peers connecting to you)"
  elif [[ $inb -gt 0 ]]; then
    warn "Inbound: ${inb} — expected ≥${MIN_INBOUND_PEERS}"
  else
    err "Inbound: 0 — nobody is connecting. Check firewall/port forward"
  fi

  # Outbound
  if [[ $outb -ge $MIN_OUTBOUND_PEERS ]]; then
    ok "Outbound: ${outb} connections (connecting to ledger peers)"
  elif [[ $outb -gt 0 ]]; then
    warn "Outbound: ${outb} — expected ≥${MIN_OUTBOUND_PEERS}"
  else
    err "Outbound: 0 — not connecting to ledger. Check topology/DNS"
  fi

  # Duplex connections (full-duplex = positive for P2P efficiency)
  if [[ $duplex -gt 0 ]]; then
  ...
  # Inbound governor breakdown (if P2P)
  ...
  # Hot peers critical
  if [[ $hot -eq 0 ]]; then
    err "0 hot peers in peer selection — no active chain-sync"
  elif [[ $hot -lt 5 ]]; then
    warn "Only ${hot} hot peers — few for robust propagation (target ≥10)"
  fi
fi
...
# Get list of ESTABLISHED inbound connections on port 6000
...
# Fallback via /proc/net/tcp if ss is unavailable
...
if [[ ${inbound_count:-0} -gt 0 ]]; then
  ...
  if [[ "$OUTPUT_FORMAT" == "text" ]]; then
    ...
    [[ $inbound_count -gt 10 ]] && hint "(+$((inbound_count - 10)) more)"
  fi

  # If BP_EXPECTED_IP is set, verify
  if [[ -n "$BP_EXPECTED_IP" ]]; then
    if echo "$inbound_ips" | grep -qFx "$BP_EXPECTED_IP"; then
      ok "BP connected (${BP_EXPECTED_IP})"
    else
      err "BP (${BP_EXPECTED_IP}) is NOT connected — your blocks will not propagate"
    fi
  else
    hint "Set BP_EXPECTED_IP to auto-verify that the BP is connected"
  fi

  # Other expected peer relays
  if [[ -n "$PEER_RELAY_IPS" ]]; then
    ...
    for peer_ip in "${PEER_ARR[@]}"; do
      ...
      if echo "$inbound_ips" | grep -qFx "$peer_ip"; then
        ok "Peer relay ${peer_ip} connected"
      else
        warn "Peer relay ${peer_ip} NOT connected (may connect via outbound)"
      fi
    done
  fi
else
  err "0 inbound TCP connections on :${RELAY_PORT}"
fi
...
if [[ -n "$metrics" ]]; then
  # Served counts — in P2P versions named *_counter (not *_count_int)
  ...
  if [[ $served_h -gt 0 ]] && [[ $served_b -gt 0 ]]; then
    ok "Serving: ${served_h} headers / ${served_b} blocks (peers are fetching from you)"
    [[ $served_chainsync_h -gt 0 ]] && hint "ChainSync headers served: ${served_chainsync_h}"
  elif [[ $served_h -gt 0 ]] || [[ $served_chainsync_h -gt 0 ]]; then
    warn "Serving headers (${served_h}/${served_chainsync_h}) but zero blocks (${served_b})"
  else
    err "0 served headers/blocks — relay is not propagating the chain"
  fi

  # ── Block fetch quality (how fast it downloads blocks) ──
  ...
  if [[ -n "$blockdelay" ]]; then
    ...
    if [[ $delay_ms -lt 500 ]]; then
      ...
      warn "Block fetch delay: ${delay_ms}ms — slow for relay"
    else
      err "Block fetch delay: ${delay_ms}ms — block propagation at risk"
    fi
  fi

  # CDF: percentage of blocks in <1s, <3s, <5s (critical for propagation quality)
  if [[ -n "$cdf_one" ]]; then
    ...
    if [[ $cdf_one_int -ge 95 ]]; then
      ...
      warn "Block diffusion: ${cdf_one_pct}% <1s (target ≥95%)"
    else
      err "Block diffusion slow: only ${cdf_one_pct}% blocks arrive <1s"
    fi
  fi
...
if [[ -n "$public_ip" ]]; then
  ...
  if [[ $SKIP_VALIDATION_WAIT -eq 1 ]]; then
    hint "External port check skipped (--fast). Inbound count is an indirect indicator."
  else
    # Try to connect to the public IP from the container
    # (NAT loopback test — if inbound > 0, we know for sure it works)
    if timeout 5 docker exec "$CONTAINER" sh -c \
         "</dev/tcp/${public_ip}/${RELAY_PORT}" 2>/dev/null; then
      ok "Port ${RELAY_PORT} reachable from outside (NAT loopback test passed)"
    elif [[ ${inb:-0} -gt 0 ]]; then
      ok "NAT loopback failed but you have ${inb} inbound — port is externally OK"
      hint "Your router may not support NAT loopback (normal)"
    else
      err "Port ${RELAY_PORT} NOT reachable + 0 inbound — check firewall/port forward"
    fi
  fi

  # Local listening — find the field that contains :PORT (varies across ss versions)
  ...
  if [[ -n "$listening" ]]; then
    ...
    err "Not listening on :${RELAY_PORT} internally"
  fi
else
  warn "Could not get public IP"
fi
...
if [[ -n "$metrics" ]]; then
  ...
  if [[ -n "$mem_txs" ]]; then
    ...
    # If 0 for a long time, there may be a connectivity issue
    if [[ ${mem_txs:-0} -eq 0 ]]; then
      hint "0 txs in mempool — normal during quiet periods"
    fi
  fi
fi

# ────────── 11. Critical Errors in logs ──────────
...
if [[ ${gc_count:-0} -lt 100 ]]; then
  ...
  warn "GC events (24h): ${gc_count} — elevated"
else
  err "GC events (24h): ${gc_count} — memory pressure"
fi

# Combined pattern in a single grep
...
if [[ ${crit_total:-0} -eq 0 ]]; then
  ok "No critical errors in the last 24h"
elif [[ ${crit_total:-0} -lt 10 ]]; then
  warn "${crit_total} critical errors (24h) — check if they repeat"
  hint "docker logs --since 24h ${CONTAINER} | grep -E '${all_critical_regex}' | tail -5"
else
  err "${crit_total} critical errors (24h) — likely a real problem"
fi

# Elevated network errors?
network_errors=$(echo "$logs_24h" | grep -ciE "Network.Mux.*Error|ConnectionAttemptFailed" || true)
if [[ ${network_errors:-0} -gt 100 ]]; then
  warn "${network_errors} network errors (24h) — possible instability"
fi
...
if [[ -n "$topology_json" ]]; then
  ...
  if [[ ${topology_count:-0} -gt 0 ]]; then
    ok "Topology has ${topology_count} configured peer hosts"
  else
    warn "No peer entries in the topology"
  fi

  # P2P-aware: relay should have useLedgerAfterSlot ≥ 0 (unlike BP)
  ...
  if [[ "$use_ledger_p2p" =~ ^[0-9]+$ ]] && [[ "$use_ledger_p2p" -ge 0 ]]; then
    ok "useLedgerPeers active (slot ${use_ledger_p2p}) — relay is doing peer discovery"
  elif [[ "$use_ledger_legacy" =~ ^[0-9]+$ ]] && [[ "$use_ledger_legacy" -ge 0 ]]; then
    ok "useLedgerAfterSlot active (slot ${use_ledger_legacy})"
  elif [[ "$use_ledger_legacy" == "-1" ]] || [[ "$use_ledger_p2p" == "-1" ]]; then
    warn "useLedgerAfterSlot=-1 on relay — no peer discovery (like BP)"
    hint "Normal if you are a private relay, otherwise enable it"
  else
    warn "useLedgerAfterSlot NOT set — relay may have limited peers"
  fi
  ...
  # Local roots (your BP is here)
  local_roots=$(echo "$topology_json" | jq -r '
    [.localRoots[]?.accessPoints[]?] | length
  ' 2>/dev/null)
  if [[ -n "$local_roots" ]] && [[ ${local_roots:-0} -gt 0 ]]; then
    ok "Local roots configured: ${local_roots} (likely your BP)"
  else
    warn "No local root — BP will not connect as a trusted peer"
  fi
else
  warn "Could not read the topology"
fi
...
if [[ "$OUTPUT_FORMAT" == "text" ]]; then
  ...
  if [[ $FAIL -eq 0 ]] && [[ $WARN -le 2 ]]; then
    echo -e "${G}${B} ✓ HEALTHY${N} — Relay is serving the chain correctly.\n"
    EXIT_CODE=0
  elif [[ $FAIL -eq 0 ]]; then
    echo -e "${Y}${B} ⚠ HEALTHY with caution${N} — Check the warnings.\n"
    EXIT_CODE=0
  else
    echo -e "${R}${B} ✗ UNHEALTHY${N} — ${FAIL} problems.\n"
    EXIT_CODE=1
  fi
fi
